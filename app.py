...
def FUN_adminlogin():
    if session.get("current_user", None) != "SITEADMIN":
...
def FUN_admin():
    if session.get("current_user", None) == "SITEADMIN":
        ...
        return(redirect(url_for("FUN_adminlogin"))) 
        #return abort(401)
...
def FUN_delete_user(id):
    if session.get("current_user", None) == "SITEADMIN":
        if id == "SITEADMIN": # ADMIN account can't be deleted.
            return abort(403)
...
def FUN_add_user():
    if session.get("current_user", None) == "SITEADMIN": # only Admin should be able to add user.
